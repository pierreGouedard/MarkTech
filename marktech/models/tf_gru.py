...
class TfModelGRU:
    """Predict PHQ-8 scores indicating depression from sequence of features using Gated Recurrent Units."""
    ...
    def from_dict(d_params: Dict[str, Any], meta_dataset: DatasetMeta, network: Optional[tf.keras.Model] = None):
        return TfModelGRU(meta_dataset, network=network, **d_params)
    ...
    def load(path_dir: Path):
        ...
        return TfModelGRU(**{**d_params, 'network': network, 'meta_dataset': meta_ds})
    ...
    def __create_network(
        ...
        Architecture:
        GRU -> [Attention | DENSE -> RELU] -> (DENSE -> RELU) -> DENSE -> [SIGMOID] -> OUTPUT
        ...
        # GRU + dropout layer unit
        X = tf.keras.layers.GRU(64, return_sequences=attention)(X)
        if dropout is not None:
            X = tf.keras.layers.Dropout(dropout)(X)

        if not attention:
            X = tf.keras.layers.Dense(64, name='hidden_dense_layer_1', activation="relu")(X)
        else:
            X = CustomAttention()(X)
    ...
    def build_datasets(
        ...
        """Define batch size and prefetch size of datasets for fitting.
